...
public class RedisConfig {
     ...
     * @return
     */
    @Bean(name = "specialTemplate")
    public RedisTemplate<String, Object> specialTemplate(RedisConnectionFactory factory) {
        // 创建RedisTemplate<String, Object>对象
        RedisTemplate<String, Object> template = new RedisTemplate<>();
        // 配置连接工厂
        template.setConnectionFactory(factory);
        ...
        jacksonSeial.setObjectMapper(om);
        StringRedisSerializer stringSerial = new StringRedisSerializer();
        RedisSerializer<String> specialKeySerial = new RedisSerializer<String>() {
            @Override
            public byte[] serialize(String o) throws SerializationException {
                byte[] result = stringSerial.serialize(o);
                // 调换一下顺序
            ...
            public String deserialize(byte[] bytes) throws SerializationException {
                ...
                String key = stringSerial.deserialize(bytes);
                return key;
            }
        };
        // redis key 序列化方式使用stringSerial
        template.setKeySerializer(specialKeySerial);
        // redis value 序列化方式使用jackson
        template.setValueSerializer(jacksonSeial);
        // redis hash key 序列化方式使用stringSerial
        template.setHashKeySerializer(stringSerial);
    ...
    public RedisTemplate<byte[], Object> bytesTemplate(RedisConnectionFactory factory) {
        // 创建RedisTemplate<String, Object>对象
        ...
        jacksonSeial.setObjectMapper(om);
        StringRedisSerializer stringSerial = new StringRedisSerializer();
        RedisSerializer<byte[]> specialKeySerial = new RedisSerializer<byte[]>() {
            @Override
